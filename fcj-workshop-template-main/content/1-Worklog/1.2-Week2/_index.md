...
4.  DATA COLLECTION (per client)
    Worker → Client AWS API → Collect metrics → Store in DynamoDB (with client\_id)
    → If critical finding → Send email alert (AWS SES)

5.  DASHBOARD DISPLAY
    Customer login → API (filtered by client\_id) → Return only their data
...
| DynamoDB hot partitions  | Medium | Low         | Proper partition key design, client\_id sharding.           |

...
4.  THU THẬP DỮ LIỆU (mỗi khách hàng)
    Worker → API AWS của Khách hàng → Thu thập metrics → Lưu vào DynamoDB (với client\_id)
    → Nếu phát hiện quan trọng → Gửi cảnh báo email (AWS SES)

5.  HIỂN THỊ DASHBOARD
    Khách hàng đăng nhập → API (lọc theo client\_id) → Trả về chỉ dữ liệu của họ
...
| Phân vùng nóng DynamoDB      | Trung bình | Thấp     | Thiết kế khóa phân vùng đúng, phân mảnh client\_id.              |
...
**A. GitHub Repository:** [https://github.com/Unvianpetronas/Cloud\_health\_dashboard](https://github.com/Unvianpetronas/Cloud_health_dashboard)
